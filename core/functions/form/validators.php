...
function validate_coordinates(string $field_input, array &$field): bool
{
    if ($field_input >= 0 && $field_input < 490) {
        return true;
    }

    $field['error'] = 'Ivestos blogos koordinates';
    return false;

}


function validate_pixel_space_available(array $form_values, array &$form): bool
{
    unset($form_values['colors']);

    if (App::$db->getRowWhere('pixels', $form_values)) {
        $form['error'] = 'Try different coordinates';
        return false;
    }
